...
CREATE TABLE public.conversations (
  ...
  mode TEXT NOT NULL CHECK (mode IN ('budget', 'health', 'school', 'agriculture')),
...
CREATE POLICY "Users can delete messages of own conversations" ON public.messages FOR DELETE
  USING (EXISTS (SELECT 1 FROM public.conversations WHERE id = conversation_id AND user_id = auth.uid()));
